...
class HAL(ContinualModel):
    ...
    def end_task(self, dataset):
        self.task_number += 1
        total_number = self.task_number * dataset.N_CLASSES_PER_TASK if dataset.SETTING != 'class-il' else self.task_number + sum(dataset.N_CLASSES_PER_TASK[:self.task_number])
        ...
        if len(self.anchors) < total_number:
    ...
    def get_anchors(self, dataset):
        ...
        for _ in range(self.finetuning_epochs):
            ...
            out = self.spare_model.forward(inputs, None)
        ...
        classes_for_this_task = np.unique(dataset.train_loader.dataset.labels_all)
        ...
        for a_class in classes_for_this_task:
            ...
            for i in range(self.anchor_optimization_steps):
                ...
                #print("fff", self.spare_model(e_t.unsqueeze(0), None).size(), torch.tensor([a_class]).unsqueeze(dim=0).unsqueeze(dim=2).repeat((1, 256, 256)).size())
                loss = -torch.sum(self.loss(self.spare_model(e_t.unsqueeze(0), None), torch.tensor([a_class]).unsqueeze(dim=0).unsqueeze(dim=2).repeat((1, 256, 256)).long().to(self.device)))
                loss.backward()
                cum_loss += loss.item()
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        total_number = k * self.dataset.N_CLASSES_PER_TASK if self.dataset.SETTING != 'class-il' else k + sum(self.dataset.N_CLASSES_PER_TASK[:k])
        assert len(self.anchors) == total_number

        if len(self.anchors) > 0:
            first_loss = loss.item()
            with torch.no_grad():
                pred_anchors = self.forward(self.anchors)
