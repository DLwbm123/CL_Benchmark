...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument('--gamma', type=float, default=0.5, #required=True,
                        help='Across-batch update gamma parameter.')
    parser.add_argument('--batch_num', type=int, default=1, #required=True,
    ...
    def draw_batches(self, inp, lab):
        ...
        for i in range(self.args.batch_num):
            if not self.buffer.is_empty():
                ...
                labels = torch.cat((buf_labels, lab))
                batches.append((inputs.to(self.device), labels.to(self.device)))
            else:
                batches.append((inp.to(self.device), lab.to(self.device)))
        return batches
