...
class Lwf(ContinualModel):
    ...
    def begin_task(self, dataset):
        ...
        if self.current_task > 0:
            ...
            if dataset.SETTING == 'class-il':
                #opt = SGD(self.net.last[str(self.current_task)].parameters(), lr=self.args.lr)
                opt = SGD(self.net.last.parameters(), lr=self.args.lr)
            else:
                opt = SGD(self.net.last.parameters(), lr=self.args.lr)
            for epoch in range(self.args.n_epochs):
                for i, data in enumerate(dataset.train_loader):
                    ...
                    outputs = self.net.logits(feats, self.current_task)
            ...
            logits = []
            print("dataset len", len(dataset.train_loader.dataset))
            with torch.no_grad():
                for i in range(0, len(dataset.train_loader.dataset), self.args.batch_size):
                    ...
                    log = self.forward(inputs.to(self.device)).cpu()
                    logits.append(log)
    ...
    def observe(self, inputs, labels, not_aug_inputs, logits=None):
        ...
        if logits is not None:
            ...
            loss += self.args.alpha * modified_kl_div(smooth(self.soft(logits).to(self.device), 2, 1),
                                                      smooth(self.soft(outputs), 2, 1))
