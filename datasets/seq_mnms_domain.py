...
class MnMs_DomainCL_Dataset(Dataset):
    def __init__(self, task_name, mode, patch_size):
        self.task = task_name
        root_pro = "/data4/CL_Seg_data/MnMs_domain/"
        if task_name == "1":
            task_h5 = os.path.join(root_pro, "A" + ".h5")  # BIDMC
        elif task_name == "2":
            task_h5 = os.path.join(root_pro, "B" + ".h5")  # UCL
        elif task_name == "3":
            task_h5 = os.path.join(root_pro, "C" + ".h5")  # ISBI
        elif task_name == "4":
            task_h5 = os.path.join(root_pro, "D" + ".h5")  # ISBI
...
def MnMsDomain_Gen(task_names, dataset, patch_size=256):
...
class SequentialMnMsDomain(ContinualDataset):
    NAME = 'seq-mnmsdomain'
    SETTING = 'domain-il'
    ...
    def get_data_loaders(self, t):
        train_splits, test_splits = MnMsDomain_Gen(["1", "2", "3", "4"], dataset=MnMs_DomainCL_Dataset)
    ...
    def get_backbone(args):
        ...
        if args.multihead:
            ...
            for task in range(N_TASKS):
                model.last[str(task)] = nn.Conv2d(32, N_CLASSES_PER_TASK, 1, 1)
...
if __name__ == '__main__':
    dataset = MnMs_DomainCL_Dataset("1", "train", 256)
