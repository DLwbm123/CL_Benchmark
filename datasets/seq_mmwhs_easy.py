...
class MMWHSCL_Dataset(Dataset):
    def __init__(self, task_name, mode, patch_size):
        self.shift = 0
        self.task = task_name
        root_heart = "/root/zhq/data/MMWHS_Incremental/"
        self.mode = mode
        if task_name == "1":
            task_h5 = os.path.join(root_heart, "myo_lv_la" + ".h5")  # BIDMC
        elif task_name == "2":
            task_h5 = os.path.join(root_heart, "myo_lv_la_ra_rv" + ".h5")  # UCL
        elif task_name == "3":
            task_h5 = os.path.join(root_heart, "myo_lv_la_ra_rv_ao_pa" + ".h5")  # ISBI
        elif task_name == "whole_heart":
            task_h5 = os.path.join('/root/zhq/data/MMWHS/', "whole_heart_test" + ".h5")  # ISBI
...
class SequentialMMWHSeasy(ContinualDataset):
    ...
    def get_data_loaders(self, t):
        ...
        if t < 2:
            ...
            self.next_test_loaders = DataLoader(next_test_dataset,batch_size=2, shuffle=False)
        else:
