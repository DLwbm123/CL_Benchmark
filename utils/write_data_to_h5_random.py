...
data_p = "E:/Continual Learning Datasets/original datasets/Prostate/data/UCL/"
savep = "E:/Continual Learning Datasets/processed datasets/NEW_Pro/UCL.h5"
...
ims = np.zeros((384, 384, 1))
labels = np.zeros((384, 384, 1))

for file in all_cases:
    if "Seg" not in file:
        filep = os.path.join(data_p, file)
        gtp = os.path.join(data_p, file[:-7]+"_Segmentation.nii.gz")
